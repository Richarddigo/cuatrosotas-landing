...
import { t, supportTranslations } from '@/i18n/translations';
        ...
        <main id="main-content">
            ...
            <section className="page-intro" aria-labelledby="contact-title">
                <div className="container">
                    <h2 id="contact-title">{t(supportTranslations.contactTitle, lang)}</h2>
                    <p>{t(supportTranslations.contactDesc, lang)}</p>
                    <div className="contact-channels-grid">
                        <div className="contact-channel">
                            <span className="icon" aria-hidden="true">🔧</span>
                            <strong>{es ? 'Soporte técnico y Privacidad' : 'Technical Support & Privacy'}</strong>
                            <a href="mailto:support@cojauny.com">support@cojauny.com</a>
                        </div>
                        <div className="contact-channel">
                            <span className="icon" aria-hidden="true">💡</span>
                            <strong>{t(supportTranslations.suggestions, lang)}</strong>
            ...
            <section className="section bg-light" aria-labelledby="times-title">
                <div className="container">
                    <h2 id="times-title" className="section-title">
                        {t(supportTranslations.responseTimes, lang)}
                    </h2>
                    <div className="modes-grid">
                        <div className="mode-card">
                            <span className="mode-icon" aria-hidden="true">🔧</span>
                            <div>
                                <h3>{t(supportTranslations.techSupportTime, lang)}</h3>
                                <p>{t(supportTranslations.techSupportDesc, lang)}</p>
                            </div>
                        </div>
                        <div className="mode-card">
                            <span className="mode-icon" aria-hidden="true">🔒</span>
                            <div>
                                <h3>{t(supportTranslations.gdprTime, lang)}</h3>
                                <p>{t(supportTranslations.gdprDesc, lang)}</p>
                            </div>
                        </div>
                        <div className="mode-card">
                            <span className="mode-icon" aria-hidden="true">💡</span>
                            <div>
                                <h3>{t(supportTranslations.suggestionsTime, lang)}</h3>
                                <p>{t(supportTranslations.suggestionsDesc, lang)}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section className="section" aria-labelledby="faq-game-title">
                <div className="container">
                    <h2 id="faq-game-title" className="section-title">
                        {t(supportTranslations.faqGameTitle, lang)}
                    </h2>
            ...
            <section className="section" aria-labelledby="faq-privacy-title">
                <div className="container">
                    <h2 id="faq-privacy-title" className="section-title">
                        {t(supportTranslations.faqPrivacyTitle, lang)}
                    </h2>
                    ...
                    {es ? (
                        <>
                            ...
                            <div className="faq-item">
                                ...
                                <p>No. Nunca vendemos datos personales a terceros. Consulta nuestra <Link href="/privacy">Política de Privacidad</Link> para más detalles.</p>
                            </div>
                            <div className="faq-item">
                                <h3>¿Qué datos recoge la app?</h3>
                                <p>Correo electrónico y nombre (de Google Sign-In), estadísticas de juego y datos técnicos para mejorar la app. Los usuarios gratuitos también generan datos de publicidad (con consentimiento). Consulta la <Link href="/privacy">Política de Privacidad</Link> completa.</p>
                            </div>
                        </>
                    ) : (
                        <>
                            ...
                            <div className="faq-item">
                                ...
                                <p>No. We never sell personal data to third parties. See our <Link href="/privacy">Privacy Policy</Link> for full details.</p>
                            </div>
                            <div className="faq-item">
                                <h3>What data does the app collect?</h3>
                                <p>Email and name (from Google Sign-In), game statistics, and technical data to improve the app. Free users also generate advertising data (with consent). See the full <Link href="/privacy">Privacy Policy</Link>.</p>
                            </div>
                        </>
                    )}

                    <div style={{ marginTop: '2rem' }}>
                        <h2 className="section-title">{t(supportTranslations.legalDocs, lang)}</h2>
                        <div className="links-grid">
                            <Link href="/privacy" className="link-card">
                                <span className="icon" aria-hidden="true">🔒</span>
                                <h3>{t(supportTranslations.privacyPolicyTitle, lang)}</h3>
                                <p>{t(supportTranslations.privacyPolicyDesc, lang)}</p>
                            </Link>
                            <Link href="/terms" className="link-card">
                                <span className="icon" aria-hidden="true">📄</span>
                                <h3>{t(supportTranslations.termsTitle, lang)}</h3>
                                <p>{t(supportTranslations.termsDesc, lang)}</p>
                            </Link>
                        </div>
                    </div>
                </div>
